<!-- resources/views/reports/receipt.blade.php -->
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<body>
    ...
    <table>
        <tr>
            <td colspan="2">Pelanggan: {{ $order->pelanggan->nama }}</td>
        </tr>
        <tr>
            <td colspan="2">Status: {{ ucfirst($order->status) }}</td>
        </tr>
        <tr>
            <td colspan="2">Pembayaran: {{ $order->metode_pembayaran == 'cash' ? 'Tunai' : 'Transfer' }}</td>
        </tr>
    </table>
    ...
    <table>
        <tr>
            <th>TOTAL</th>
            <th class="right">Rp {{ number_format($order->total_harga, 0, ',', '.') }}</th>
        </tr>
    </table>
