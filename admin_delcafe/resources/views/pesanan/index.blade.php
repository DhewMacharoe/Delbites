...
@section('title', 'Daftar Pesanan - DelBites')
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $p->id }}">
                                                <li>
                                                    <a class="dropdown-item detail-btn" href="#" data-bs-toggle="modal" data-bs-target="#detailModal" data-id="{{ $p->id }}">
                                                        <i class="fas fa-eye me-2"></i> Lihat Detail
                                                    </a>
                                                </li>
                                                @if(in_array($p->status, ['dibayar', 'diproses', 'selesai']))
                                                <li>
                                                    <a class="dropdown-item" href="{{ route('pesanan.cetak-struk', $p->id) }}" target="_blank">
                                                        <i class="fas fa-print me-2"></i> Cetak Struk
                                                    </a>
                                                </li>
                                                @endif
                                                <li>
                                                    <a class="dropdown-item" href="https://wa.me/{{ $p->pelanggan->telepon }}" target="_blank">
                                                        <i class="fab fa-whatsapp me-2"></i> Hubungi Pelanggan
                                                    </a>
                                                </li>
                                                @if(!in_array($p->status, ['selesai', 'dibatalkan']))
                                                <li>
                                                    <form action="{{ route('pesanan.batalkan', $p->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin membatalkan pesanan ini?')">
                                                        @csrf
                                                        @method('PUT')
        ...
        detailModal.addEventListener('show.bs.modal', function(event) {
            ...
            fetch(`/pesanan/${id}`)
                ...
                .then(data => {
                    ...
                    document.getElementById('teleponPelanggan').textContent = data.pelanggan.telepon || '-';
                    document.getElementById('tanggalPesanan').textContent = new Date(data.created_at).toLocaleString('id-ID');
                    ...
                    data.detail_pemesanans.forEach(detail => {
